...
const getAllActivityByRole = asyncHandler(async (req, res) => {
  try {
    ...
    if (!activities.length) {
      return res
        .status(404)
        .json({ message: `No activities found for role: ${role}` });
    }
...
const getAdminActivity = async (req, res) => {
  ...
  try {
    ...
    if (!activityLogs.length) {
      return res
        .status(404)
        .json({ message: "No activity found for this admin" });
    }
...
const getEachActivity = async (req, res) => {
  ...
  try {
    const activityLogs = await ActivityLog.find({ _id: id }).populate(
      "user",
      "firstname",
    );
    if (!activityLogs.length) {
...
const updateActivityLog = async (req, res) => {
  try {
    ...
    if (typeof Isread !== "boolean") {
      return res
        .status(400)
        .json({ message: "Invalid value for Isread. It must be a boolean." });
    }
