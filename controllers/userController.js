...
const createAppUser = asyncHandler(async (req, res) => {
  ...
  // Check if a role is provided, if not default to "user"
  const userRole = role ? role : "user";  // Default to "user" if no role is specified
  ...
  const newUser = new User({
    ...
    password, // This will be hashed in the User model pre-save middleware
    mobile,
    role: userRole,  // Assign role based on input
  });

  // Generate OTP for email verification
  const otp = newUser.createEmailVerificationOTP(); // Assuming this function exists in your model
  await newUser.save(); // Save the user

  // Send the OTP via email (you should already have a sendEmail function set up)
...
const loginUserCtrl = asyncHandler(async (req, res) => {
  ...
  // Check if email is verified BEFORE checking the password
  if (!findUser.isEmailVerified) {
    // Send a specific, clear error message
    res.status(401); // 401 Unauthorized is appropriate here
...
const updatedUser = asyncHandler(async (req, res) => {
  try {
    ...
    console.log('Updated data:', updatedData.profileInfo);

    const user = await User.findByIdAndUpdate(userId, updatedData.profileInfo, {
      new: true, 
      runValidators: true,
