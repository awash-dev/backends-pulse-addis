...
const createProduct = asyncHandler(async (req, res) => {
  try {
    ...
    await ActivityLog.create({
      ...
      user: newProduct?.PostedByuserId || req.user?._id,
      details: { newProduct },
    });
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Map colorImagePairs if provided (same as create handler)
    if (req.body.colorImagePairs) {
      req.body.colors = req.body.colorImagePairs.map((pair) => ({
        color: pair.color,
        images: pair.images,
      }));
      delete req.body.colorImagePairs; // avoid storing redundant field
    }
    ...
    await ActivityLog.create({
      ...
      user: updateProduct?.PostedByuserId || req.user?._id,
...
const mongoose = require("mongoose"); // add at top if not already present elsewhere

const getaProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  // guard against invalid ObjectId strings to avoid CastError
  if (!mongoose.Types.ObjectId.isValid(id)) {
    // log for diagnostics but don't treat as server error
    console.warn("getaProduct called with invalid id:", id);
    return res.status(400).json({ message: "Invalid product identifier" });
  }

  try {
    const product = await Product.findById(id).populate("store", "storeName");
    if (!product) {
...
const getAllProduct = asyncHandler(async (req, res) => {
  try {
    ...
    queryObj.ProductApproved = "approved"; // Only approved products (lowercase to match model)
...
const NotApprovedProducts = asyncHandler(async (req, res) => {
  try {
    ...
    const skip = (page - 1) * limit;
    let query = Product.find({ ProductApproved: "pending" })
      .skip(skip)
      .limit(limit);
    if (req.query.sort) {
      const sortBy = req.query.sort.split(",").join(" ");
    ...
    const totalCount = await Product.countDocuments({
      ProductApproved: { $regex: /^pending$/i }, // Case-insensitive match for 'pending'
    });
...
const RejectedProducts = asyncHandler(async (req, res) => {
  try {
    ...
    const limit = req.query.limit || 10;
    const skip = (page - 1) * limit;
    let query = Product.find({ ProductApproved: "rejected" })
      .skip(skip)
      .limit(limit);
    if (req.query.sort) {
      const sortBy = req.query.sort.split(",").join(" ");
    ...
    const totalCount = await Product.countDocuments({
      ProductApproved: { $regex: /^Rejected$/i },
    });
    console.log(totalCount);
    const products = await query;
...
const updateProductStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updatedProduct = await Product.findByIdAndUpdate(
      id,
      {
        ProductApproved: ProductApproved.toLowerCase(),
        rejectionReason,
      },
      { new: true },
    );
    ...
    await ActivityLog.create({
      action: `Update Product Approval Status: ${ProductApproved}`,
      resource: "Product",
      resourceId: updatedProduct?._id,
      user: req.user?._id || updatedProduct.PostedByuserId,
      details: { ProductApproved, rejectionReason, updatedProduct },
    });
...
const fetchRecentProducts = async (req, res) => {
  try {
    ...
    if (recentProducts.length === 0) {
      return res
        .status(202)
        .json({ message: "No products found for this merchant." });
    }
...
const EachMerchantProducts = async (req, res) => {
  try {
    const { id } = req.params;
    console.log(id);
    const recentProducts = await Product.find({ PostedByuserId: id }).sort({
      createdAt: -1,
    });
...
const AllEachMerchantProducts = async (req, res) => {
  try {
    const recentProducts = await Product.find().sort({ createdAt: -1 });
