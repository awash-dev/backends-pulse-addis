...
console.log("Cloudinary config -> cloud_name:", CLOUDINARY_CLOUD_NAME);
console.log("Cloudinary config -> api_key:", CLOUDINARY_API_KEY);
// (not printing api_secret for security reasons)
...
cloudinary.api.ping((err, result) => {
  if (err) {
    ...
    console.error("Please verify CLOUDINARY_API_SECRET matches the one shown in your Cloudinary dashboard and ensure the .env file is loaded correctly.");
  } else {
    console.log("Cloudinary configured, response time:", result.duration);
  }
});

function upload(file) {
  const timestamp = Math.floor(Date.now() / 1000);
  console.log("cloudinary upload timestamp", timestamp);
